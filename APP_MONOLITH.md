...
def calculate_threat_score(t: dict) -> int:
    """Calculate an AI heuristic competitive threat score (0-100)."""
    score = 0
    ...
    major_sponsors = ["pfizer", "gsk", "sanofi", "merck", "moderna", "astrazeneca", "jnj", "johnson", "biontech", "novartis"]
...
def show_interactive_df(df: pd.DataFrame, key: str, height: int = 420):
    """Render an interactive workspace table with persistable analyst notes."""
    notes = _load_analyst_notes()
    
    # Bypass PyArrow issues
    safe_df = df.copy()
    
    # Drop "Locations" from tabular view to keep it clean if it exists
    if "Locations" in safe_df.columns:
        safe_df = safe_df.drop(columns=["Locations"])
        
    # Prepend Analyst Columns
    safe_df.insert(0, "Favorite", safe_df["NCT ID"].apply(lambda x: notes.get(str(x), {}).get("Favorite", False)))
    safe_df.insert(1, "Analyst_Note", safe_df["NCT ID"].apply(lambda x: notes.get(str(x), {}).get("Note", "")))
    
    # We want boolean for Favorite, string for Note.
    # Convert others to float if numeric (like Threat Score), else string
    for col in safe_df.columns:
        if col not in ["Favorite", "Analyst_Note", "Threat Score"]:
            safe_df[col] = safe_df[col].apply(lambda x: "" if pd.isna(x) else str(x))
            
    # Determine which columns are disabled (cannot be edited)
    disabled_cols = [c for c in safe_df.columns if c not in ["Favorite", "Analyst_Note"]]
    
    try:
        if _supports_width_string():
            edited_df = st.data_editor(safe_df, width="stretch", height=height, key=f"editor_{key}", disabled=disabled_cols)
        else:
            edited_df = st.data_editor(safe_df, use_container_width=True, height=height, key=f"editor_{key}", disabled=disabled_cols)
    ...
    try:
        while page_count < max_pages:
            ...
            for s in studies:
                ...
                trial_obj["Threat Score"] = calculate_threat_score(trial_obj)
...
def fetch_pipeline_publications(query: str, max_items: int = 5):
    """Fetch latest vaccine publications from PubMed + FDA press releases."""
    items = []
    
    # --- PubMed: English-only, recent, clinical-trial-biased ---
    try:
        # Build a query: vaccine name + clinical trial context, English only, last 2 years
    ...
    try:
        ...
        if not entries:
            entries = root.findall('.//item')  # Fallback to RSS 2.0
...
def _fetch_faers_count(brand_name: str) -> int:
    """Get total FAERS adverse event report count for a vaccine from openFDA."""
    try:
...
def fetch_openfda_data(vaccine_name: str):
    ...
    def _try_ndc_search(search_term: str):
        """Attempt a single openFDA NDC search."""
        try:
    ...
    # Strategy 2: Try known synonyms from our synonym index
    if not results:
        norm = _norm_txt(vaccine_name)
    ...
    # Strategy 3: Try the manufacturer index to get a brand name hint
    if not results:
        for mfr, names in _VACCINE_MANUFACTURER_DATA.items():
            for name in names:
    ...
    # Pick the best result (prefer VACCINE product_type)
    best = results[0]
    for r in results:
    ...
    ingredient_str = "; ".join([f"{i.get('name', '')} ({i.get('strength', '')})" for i in ingredients]) if ingredients else "Not listed"
    
    # Parse marketing date
...
def create_country_heatmap(df: pd.DataFrame):
    """Create global trial footprint heatmap."""
    try:
        import plotly.express as px
        country_counts = {}
        # df might not have Locations column if loaded from old cache, handle gracefully
                ...
                with st.spinner(f"Step 1/2: Finding trials for '{vaccine_name}'..."):
                    ...
                    for term in search_terms:
                        ...
                        for s in studies:
                            ...
                            trial_obj_v["Threat Score"] = calculate_threat_score(trial_obj_v)
